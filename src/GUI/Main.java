...
//                disassembler.Disassemble(desktopPane.getSelectedFrame().getTitle().replace(".tri", ".tam"));  // set TAM CODE tab
                ((FileFrame)desktopPane.getSelectedFrame()).setTree((DefaultMutableTreeNode)treeVisitor.visitProgram(compiler.getAST(), null));
//                ((FileFrame)desktopPane.getSelectedFrame()).setTable(tableVisitor.getTable(compiler.getAST()));  // set Table Details tab
                
                runMenuItem.setEnabled(false);  // disable running code
                buttonRun.setEnabled(false);
            } else {
